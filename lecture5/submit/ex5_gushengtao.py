...
def load_dataset():
    ...
    # ===================== YOUR CODE HERE =====================================
    os.chdir('C:\\Users\Administrator\Desktop\lecture5\datasets')
    f=h5py.File('test_catvnoncat.h5','r+')
    g=h5py.File('train_catvnoncat.h5','r+')
    train_set_x_orig=g["train_set_x"][:]
    train_set_y=(g["train_set_y"][:]).reshape(1,209)# 如何直接生成列向量
    test_set_x_orig=f["test_set_x"][:]
    test_set_y=(f["test_set_y"][:]).reshape(1,50)
    classes=(g["list_classes"][:]).reshape(1,2)
...
def optimize(w, b, X, Y, num_iterations, learning_rate, print_cost=False):
    ...
    # ===================== YOUR CODE HERE =====================================
        a=1.0/(1+np.exp(-(np.dot(w.T,X)+b)))
        costs.append(-1.0/m*(np.sum((Y*np.log(a)+(1-Y)*np.log(1-a)))))
        dw=1.0/m*np.dot(X,(a-Y).T)
        db=1.0/m*np.sum(a-Y)
...
def predict(w, b, X):
    ...
    # ===================== YOUR CODE HERE =====================================
    Y_prediction=1.0/(1+np.exp(-(np.dot(w.T,X)+b)))
...
def plot(d):
    ...
    # ===================== YOUR CODE HERE =====================================
    x = range(2000) 
    y = [costs[i] for i in x]  
    plt.plot(x, y,'r')
    plt.legend()
    plt.title('Demo Figure')
    plt.xlabel('iteration(per hunderds')
    plt.ylabel('costs')
    plt.show()
    # ==========================================================================
...
if __name__ == "__main__":
    ...
    print("The shape of the variable classes is", classes.shape)
    ...
    os.chdir('C:\\Users\Administrator\Desktop\lecture5')
