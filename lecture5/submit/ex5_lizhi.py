...
def load_dataset(datasets1,datasets2):
    ...
    # ===================== YOUR CODE HERE =====================================
    with h5py.File(datasets1, 'r') as train,h5py.File(datasets2, 'r') as test:  #\\?
        L1=list(train.keys())
        L2=list(test.keys())
        train_set_x_orig = train[L1[1]][:]
        train_set_y = train[L1[2]][:]
        train_set_y = (train_set_y.reshape(len(train_set_y),1)).T
        test_set_x_orig=test[L2[1]][:]
        test_set_y=test[L2[2]][:]
        test_set_y = (test_set_y.reshape(len(test_set_y),1)).T
        classes=train[L1[0]][:]
        classes = (classes.reshape(len(classes),1)).T
...
def flatten_shape(train_set_x_orig, test_set_x_orig):
    ...
    # ===================== YOUR CODE HERE =====================================
    train_set_x_flatten = (train_set_x_flatten[:].reshape(len(train_set_x_flatten),-1)).T
    test_set_x_flatten = (test_set_x_flatten[:].reshape(len(test_set_x_flatten),-1)).T
    # ==========================================================================
...
def propagate(w, b, X, Y):#正反向传播
    ...
    cost = (Y.dot(np.log(A).T)+(1-Y).dot(np.log(1-A).T))/(-m)
    dw = X.dot((A-Y).T)/m
    db = np.sum(A-Y)/m
    # ==========================================================================
...
def optimize(w, b, X, Y, num_iterations, learning_rate, print_cost=False):#参数优化
    costs = []
    dw = w
    db = b
...
def plot(d):
    ...
    # ===================== YOUR CODE HERE =====================================
    y = costs
    plt.plot(y, color="blue", linewidth=1, linestyle="-")
    plt.xlabel('iterations(per hundreds)')
    plt.ylabel('cost')
    plt.title('Learning rate = 0.005')
    plt.xticks(np.linspace(0,2000,9,endpoint=True))#9个点
    plt.ylim(0.1,0.95)
    plt.yticks(np.linspace(0.1,0.9,9,endpoint=True))
    plt.show()
    # ==========================================================================
...
if __name__ == "__main__":
    ...
    # ======================== Part 1: Loading Data ============================
    print('Loading Data ...')
    train_set_x_orig, train_set_y, test_set_x_orig, test_set_y, classes = load_dataset('C:\work/train_catvnoncat.h5','C:\work/test_catvnoncat.h5')#\\?
    ...
    my_image = "loin.jpg"   # change this to the name of your image file
    num_px = 64
