...
def gradientDescent(X, y, theta, alpha, num_iters):
    ...
    # ================== YOUR CODE HERE =============================
        h = np.dot(theta.T,X.T)
        theta[0] = theta[0] - alpha*np.sum(h-y.T)/m
        theta[1] = theta[1] - alpha*np.dot((h-y.T),X[:,1:2])/m
        theta[2] = theta[2] - 2*alpha*np.dot((h-y.T),X[:,2:3])/m
        J_log.append(computeCost(X, y, theta))
    # ===============================================================
    return theta, J_log
...
def predictVals(X, y, X_test, theta):
    ...
    Predict = np.dot(theta.T,x_test.T)
    u2 = np.mean(y)
    s2 = np.std(y)
    predict = (Predict*s2+u2).T           
    # ===============================================================
    return predict
