...
def featrueScaling(vals):
    ...
    # ================== YOUR CODE HERE =============================
    vals_norm=(vals-np.mean(vals,axis=0))/(np.std(vals,axis=0))
...
def gradientDescent(X, y, theta, alpha, num_iters):
    ...
    # ================== YOUR CODE HERE =============================
        q=np.dot(X,theta)-y
        p=np.multiply(q,X)
        r=alpha/m*np.sum(p,axis=0)
        theta=theta-r.reshape(3,1)
        x=np.dot(X,theta)-y
        J_log.append(sum(0.5/m*(np.multiply(x,x))))
...
def plotCost(J_log):
    # ================== YOUR CODE HERE =============================
    x = range(1500) 
    y = [J_log[i] for i in x]  
    plt.plot(x, y,'r',label='cost curve')
    plt.legend()
    plt.xlabel('iters num')
    plt.ylabel('J of iters num')
    plt.show()
...
def predictVals(X, y, X_test, theta):
    ...
    # ================== YOUR CODE HERE =============================
    c=np.ones(2)
    m=(X_test-np.mean(X,axis=0))/(np.std(X,axis=0))
    d=np.insert(m, 0, values=c, axis=1)
    Y=np.dot(d,theta)
    predict=np.std(y,axis=0)*Y+np.mean(y,axis=0)
