...
def featrueScaling(vals):
    ...
    # ================== YOUR CODE HERE =============================
    pass
    ui = np.mean(vals,axis = 0)
    si = np.std(vals,axis = 0)
    vals_norm = (vals - ui) / si
    # ===============================================================
    return vals_norm
...
def gradientDescent(X, y, theta, alpha, num_iters):
    ...
    # ================== YOUR CODE HERE =============================
        ...
        j2 = j1 * X[:,1:2]
        j3 = j1 * X[:,2:3]
        d = np.array([[float(sum(j1))],[float(sum(j2))],[float(sum(j3))]])
        theta -= alpha * d / m
...
def predictVals(X, y, X_test, theta):
    predict = 0
    # ================== YOUR CODE HERE =============================
    X_norm = (X_test - np.mean(X,axis = 0)) / np.std(X,axis = 0)
    m = X_test.shape[0]
    X_norm = np.hstack((np.ones((m, 1)), X_norm))
    y_norm = X_norm * theta.T
    y_sum = y_norm[:,0:1] + y_norm[:,1:2] + y_norm[:,2:3]
    predict = y_sum * np.std(y,axis = 0) + np.mean(y,axis = 0)
    # ===============================================================
    return predict
...
if __name__ == '__main__':
    ...
    # ================ Part 1: Loading Data ==================
    print('Loading Data ...')
    X, y = load('F:/Work/lecture4/lecture4/dataset_4.txt')
